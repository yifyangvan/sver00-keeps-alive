#!/bin/bash
export ACCOUNTS="$ACCOUNTS_S15"
export WXPUSHER_TOKEN="$WXPUSHER_TOKEN"
export PUSHPLUS_TOKEN="$PUSHPLUS_TOKEN"
export WXPUSHER_USER_ID="$WXPUSHER_USER_ID"
...
EOF
  ...
  if [[ "$OPERATION_RESULT" == "❌" ]]; then
    MASKED_USERNAME=$(mask_username "$USERNAME")
    MASKED_SERVER_OUT=$(mask_server_out "$SERVER")
    RESULT_SUMMARY+="❌      $index. $MASKED_USERNAME       【 $MASKED_SERVER_OUT 】 登录失败\n"
  fi
  ((index++))
done
echo "发送 WXPusher、PushPlus、Telegram 消息推送  "
send_wxpusher_message "Serv00进程通知" "$RESULT_SUMMARY"
send_pushplus_message "Serv00进程通知" "$RESULT_SUMMARY"
